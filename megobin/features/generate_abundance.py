...
# https://github.com/BigDataBiology/SemiBin/blob/main/SemiBin/generate_coverage.py
    ...
    for contig_name, lines in groupby(depth_stream, lambda ell: ell.split("\t", 1)[0]):
        ...
        if sep is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Error parsing contig name '{contig_name}' in file {bam_file} (trying to split by {sep} separator)"
                )
            except KeyError:
                raise KeyError(
                    f"Error: {sample} not found (parsing {bam_file}, the original contig name was {contig_name} and using {sep} as separator)"
                )
...
def generate_cov(
    ...
    if bed_p.wait() != 0:
        ...
        logger.critical(
            f"Running `bedtools genomecov` did not return an error, but the result is an empty file (processing {bam_file}). "
    ...
    if not is_combined:
        with atomic_write(
            os.path.join(out, "{}_data_cov.csv".format(bam_name)), overwrite=True
        ) as ofile:
        ...
        with atomic_write(
            os.path.join(out, f"{bam_name}_data_cov.csv"), overwrite=True
        ) as ofile:
            contig_cov.to_csv(ofile)
        ...
        with atomic_write(
            os.path.join(out, f"{bam_name}_data_split_cov.csv"), overwrite=True
        ) as ofile:
...
def combine_cov(
    cov_dir: str, bam_list, is_combined: bool
):  # bam_list : list[str] does not work on Python 3.7
    """
    ...
    for bam_index, bam_file in enumerate(bam_list):
        ...
        if is_combined:
            split_covs.append(
                pd.read_csv(
                    f"{cov_dir}/{bam_fname}_{bam_index}_data_split_cov.csv", index_col=0
                )
            )
...
def generate_cov_from_abundances(
    abundances,
    output,
    contig_path,
    contig_threshold=1000,
        ...
        for h, seq in fasta_iter(contig_path):
            if sep is None:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error parsing contig name '{h}' in file {abun_file} (trying to split by {sep} separator)"
                    )
                except KeyError:
    ...
    if sep is None:
        with atomic_write(
            os.path.join(output, "cov_split.csv"), overwrite=True
        ) as ofile:
