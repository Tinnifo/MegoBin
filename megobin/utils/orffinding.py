...
# https://github.com/BigDataBiology/SemiBin/blob/main/SemiBin/orffinding.py
...
def run_prodigal(fasta_path, num_process, output):
    ...
    try:
        ...
        for p in process:
            rc = p.wait()
            if rc != 0:
                sys.stderr.write(f"Error: prodigal exited with return code {rc}\n")
                sys.exit(1)

    except OSError as e:
        sys.stderr.write(f"Error: Running prodigal failed: {e}\n")
        sys.exit(1)
    ...
    with open(contig_output, "w") as f:
        for index in range(next_ix):
            with open(
                os.path.join(output, "contig_{}.faa".format(index)), "r"
            ) as infile:
                f.write(infile.read())
...
def run_fraggenescan(fasta_path: str, num_process: int, output: str):
    try:
        ...
        with open(contig_output + ".out", "w") as frag_out_log:
            # Call FragGeneScan directly because the Perl wrapper mishandles
            # paths with spaces. The binary doesn't return rc properly so
            # we cannot use check_call.
        ...
        sys.stderr.write(f"Error: Running fraggenescan failed (error: {e})\n")
        sys.exit(1)
...
def run_orffinder(fasta_path, num_process, tdir, orf_finder, prodigal_output_faa):
    ...
    if prodigal_output_faa is not None:
        ...
        sys.stderr.write(f"Error: Unknown ORF finder {orf_finder}\n")
        sys.exit(1)
