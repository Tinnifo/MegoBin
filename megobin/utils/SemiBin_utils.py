...
def check_training_type(logger, args):
    ...
    if not args.self_supervised and not args.semi_supervised:
        logger.debug(f"SemiBin will run in self supervised mode")
...
def validate_normalize_args(logger, args):
    ...
    def expect_file(f):
        ...
        if f is not None:
            if not os.path.exists(f):
                logger.error(f"Error: Expected file '{f}' does not exist\n")
    ...
    if args.cmd in ["single_easy_bin", "multi_easy_bin", "train_semi", "bin"]:
        ...
        if args.orf_finder == "fraggenescan":
            ...
            logger.warning(
                "Using FragGeneScan (--orf-finder=fraggenescan) as an ORF finder is deprecated and will be removed in the next version of SemiBin. "
        ...
        if args.prodigal_output_faa is not None:
            logger.warning(
                "Using --prodigal-output-faa is deprecated and will be removed in a future version of SemiBin"
            )
    ...
    if args.cmd in ["train_semi", "train_self"]:
        if not args.train_from_many:
            if len(args.data) > 1:
                logger.error(f"Error: Expected one data.csv file with single mode.\n")
                exit_with_error = True
            ...
            if args.cmd == "train_semi":
                if len(args.contig_fasta) > 1:
                    logger.error(f"Error: Expected one fasta file with single mode.\n")
                    exit_with_error = True
            ...
            if args.cmd == "train_semi":
                assert (
                    len(args.contig_fasta)
                    == len(args.data)
                    == len(args.data_split)
                    == len(args.cannot_link)
                ), "Must input same number of fasta, data, data_split, cannot files!"
                expect_file_list(args.cannot_link)
                expect_file_list(args.contig_fasta)
            else:
                assert len(args.data) == len(args.data_split), (
                    "Must input same number of data, data_split!"
                )
    ...
    if args.cmd in ["single_easy_bin", "multi_easy_bin"]:
        if args.sequencing_type.lower() in [
            "short",
            "short-read",
            "short_reads",
            "short-reads",
        ...
        elif args.sequencing_type.lower() in [
            "long",
            "long-read",
            "long_reads",
            "long-reads",
    ...
    if getattr(args, "write_pre_reclustering_bins", False) and not getattr(
        args, "recluster", True
    ):
        logger.error(
...
def parse_mmseqs(mmseqs_result):
    species_result = mmseqs_result.query(
        'rank_name == "species" and score > 0.95'
    ).values
    ...
    for i in range(len(species_result)):
        for j in range(i + 1, len(species_result)):
            if species_result[i][2] != species_result[j][2]:
                cannot_link_species.append((species_result[i][0], species_result[j][0]))

    cannot_link_genus = []
    for i in range(len(genus_result)):
        for j in range(i + 1, len(genus_result)):
            if genus_result[i][2] != genus_result[j][2]:
                cannot_link_genus.append((genus_result[i][0], genus_result[j][0]))
    ...
    for i in range(len(species_result)):
        ...
        for j in range(len(genus_result)):
            if genus_name != genus_result[j][2]:
                cannot_link_mix.append((species_result[i][0], genus_result[j][0]))
...
def generate_cannot_link(mmseqs_path, namelist, num_threshold, output, sample):
    ...
    mmseqs_result = mmseqs_result[mmseqs_result["contig_name"].isin(namelist)]
    ...
    with open(os.path.join(output, f"{sample}.txt"), "w") as out_cannot_link:
        for cannot in itertools.chain(
            cannot_link_species, cannot_link_genus, cannot_link_mix
        ):
            out_cannot_link.write(f"{cannot[0]},{cannot[1]}\n")


def maybe_uncompress(fafile, tdir):
    if fafile.endswith(".gz") or fafile.endswith(".bz2") or fafile.endswith(".xz"):
...
def write_bins(
    ...
    return pd.DataFrame(
        written, columns=["filename", "nbps", "nr_contigs", "N50", "L50"]
    )
...
def load_fasta(fasta_path: str, ratio: float):
    ...
    computed_min_length = 1000 if contig_bp_2500 / total_bps < ratio else 2500
...
def split_data(data, sample, separator, is_combined=True):
    ...
    part_data = data[data["contig_name"].str.contains(sample + separator, regex=False)]
...
def concatenate_fasta(
    fasta_files, min_length, output, separator, output_compression="none"
):
    """
...
def maybe_crams2bams(
    ...
    for bam in bams:
        if bam.endswith(".cram"):
            obam = (
                pathlib.PurePath(odir) / pathlib.PurePath(bam).with_suffix(".bam").name
            )
