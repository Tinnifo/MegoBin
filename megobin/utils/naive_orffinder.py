...
# https://github.com/BigDataBiology/SemiBin/blob/main/SemiBin/naive_orffinder.py
...
# Cap process count to avoid the slowdown described there.
MAX_PROCESS_ORFS = 24
...
def find_orfs_fwd(seq, accept_incomplete=False):
    ...
    if accept_incomplete:
        ...
        active = [-1, -1, -1]
    orfs = []
    positions = findall(seq, START_CODONS + STOP_CODONS)
    for i in positions:
        ix = i % 3
        ...
        for ix in range(3):
            if active[ix] != -1:
                if len(seq) - active[ix] > MIN_LEN and is_atcg(seq[active[ix] :]):
                    orfs.append(
                        ORFPos(active[ix], len(seq) - (len(seq) - ix) % 3, False)
                    )
    return orfs
    ...
    with open(oname, "w") as out:
        if num_process == 1:
            ...
            with ctx.Pool(processes=min(num_process, MAX_PROCESS_ORFS)) as p:
                outs = p.imap(get_orfs, fasta.fasta_iter(fasta_path), chunksize=8)
                for o in outs:
                    out.write(o)
    return oname
